...
export const Route = createFileRoute("/services")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "E-commerce development, custom web apps, UI/UX design, payment integration and cloud deployment by SandroGen Tech." },
  ...
  return (
    <>
          ...
          {services.map((s, i) => (
            <div key={s.title} className="glass-card glass-card-hover p-10">
              ...
              <div className="grid grid-cols-2 gap-2">
                {s.items.map(it => (
                  <div key={it} className="text-sm text-white/70 flex items-center gap-2 py-2 border-t border-white/5">
      ...
      <section className="container-x py-32 border-t border-white/5 mt-20">
        ...
        <h2 className="font-display text-4xl md:text-6xl font-semibold mt-4 mb-16 max-w-3xl">Built on a modern, battle-tested stack.</h2>
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
          {Object.entries(techStack).map(([cat, items]) => (
            <div key={cat} className="glass-card p-8">
              <h4 className="font-mono text-xs uppercase tracking-[0.2em] text-[#D4FF00] mb-6">{cat}</h4>
              <ul className="space-y-3">
                {items.map(i => (
                  <li key={i} className="text-lg font-display">{i}</li>
                ))}
              </ul>
            </div>
          ))}
