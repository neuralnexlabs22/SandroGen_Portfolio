  ...
  head: () => ({
    meta: [
      ...
      { title: "SandroGen Tech — Premium Web Development & E-Commerce" },
      {
        name: "description",
        content:
          "SandroGen Tech builds high-performance websites, e-commerce platforms and custom business apps for ambitious brands.",
      },
      ...
      },
      {
        property: "og:image",
        content:
          "https://pub-bb2e103a32db4e198524a2e9ed8f35b4.r2.dev/75d55ed5-795d-4f1a-ae87-17eb8fa27a92/id-preview-b373e1dd--9300235c-7614-42ae-89a0-611e2adbc2f6.lovable.app-1780941765861.png",
      },
      {
        name: "twitter:image",
        content:
    ...
    <header className="fixed top-0 inset-x-0 z-50 backdrop-blur-xl bg-[#0A0A0A]/70 border-b border-white/5">
      <div className="container-x flex items-center justify-between h-16">
        <Link to="/" className="flex items-center gap-2 group">
          <span className="w-8 h-8 rounded-lg bg-[#D4FF00] flex items-center justify-center text-black font-bold text-sm">
            S
          </span>
          <span className="font-display font-semibold tracking-tight">
            SandroGen<span className="text-[#D4FF00]">.</span>
          </span>
        </Link>
        <nav className="hidden md:flex items-center gap-1">
          {navItems.map((n) => (
            <Link
              ...
              activeProps={{
                className: "px-4 py-2 text-sm text-[#D4FF00] rounded-full bg-white/5",
              }}
          ...
          <nav className="container-x py-4 flex flex-col gap-1">
            {navItems.map((n) => (
              <Link
                ...
                className="px-4 py-3 text-white/80 hover:text-[#D4FF00]"
              >
                {n.label}
              </Link>
            ))}
      ...
      <div className="container-x py-16 grid md:grid-cols-4 gap-10">
        <div className="md:col-span-2">
          ...
          <p className="text-white/60 max-w-md text-sm leading-relaxed">
            Premium software development for ambitious brands. We build websites, e-commerce
            platforms and custom business applications that drive growth.
          </p>
        </div>
        <div>
          <h4 className="text-xs uppercase tracking-[0.2em] text-white/40 mb-4 font-mono">
            Explore
          </h4>
          <ul className="space-y-2 text-sm">
            {navItems.map((n) => (
              <li key={n.to}>
                <Link to={n.to} className="text-white/70 hover:text-[#D4FF00] transition">
                  {n.label}
                </Link>
              </li>
            ))}
          ...
          <ul className="space-y-2 text-sm">
            <li>
              <a
                ...
                className="text-white/70 hover:text-[#D4FF00]"
              >
                rohithlakshman999@gmail.com
              </a>
            </li>
            <li>
              <a href="https://wa.me/917348975878" className="text-white/70 hover:text-[#D4FF00]">
                +91 73489 75878
              </a>
            </li>
            <li>
