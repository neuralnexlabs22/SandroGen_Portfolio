...
export const Route = createFileRoute("/projects")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Selected e-commerce, fashion, sports and business software projects built by SandroGen Tech." },
      { property: "og:title", content: "Selected Work — SandroGen Tech" },
      { property: "og:description", content: "Premium e-commerce and custom application projects." },
    ],
  }),
  component: Projects,
});
          ...
          <article key={p.id} className="glass-card glass-card-hover p-8 md:p-12 group">
            <div className="grid md:grid-cols-12 gap-8 items-start">
              ...
              <div className="md:col-span-7">
                <div className="text-xs font-mono uppercase tracking-[0.2em] text-white/40 mb-3">{p.industry}</div>
                ...
                <div className="flex flex-wrap gap-2">
                  {p.features.map(f => (
                    <span key={f} className="text-xs px-3 py-1.5 rounded-full border border-white/10 text-white/60">{f}</span>
                  ))}
                </div>
              </div>
              <div className="md:col-span-4 space-y-4">
                <div>
                  <div className="text-xs font-mono uppercase tracking-[0.2em] text-white/40 mb-2">Highlights</div>
                  <ul className="space-y-1 text-sm">
                    {p.highlights.map(h => <li key={h} className="text-white/75">— {h}</li>)}
                  </ul>
                </div>
                {p.url ? (
                  <a href={p.url} target="_blank" rel="noreferrer" className="btn-primary text-sm">Visit site ↗</a>
                ) : (
                  <span className="inline-block text-xs font-mono px-3 py-1.5 rounded-full bg-white/5 text-white/50">Internal project</span>
                )}
              </div>
            </div>
          </article>
